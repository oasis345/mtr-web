...
// 새로운 샘플 데이터
const sampleData2: ChartDataPoint[] = [
...
const sampleData3: ChartDataPoint[] = [
...
export const Chart: React.FC = () => {
  ...
  const [lastMessageFromWeb, setLastMessageFromWeb] = useState<string>('');
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.webviewContainer}>
        <WebView
          ref={webviewRef}
          source={{ uri: EMBEDDABLE_CHART_URL }}
          onLoadEnd={() => setIsWebViewReady(true)} // WebView 로딩 완료 시 상태 업데이트
      ...
      <View style={styles.controlPanel}>
        ...
        <View style={styles.buttonGroup}>
          <Button
            title="Update to Data 2"
            onPress={() => updateChartData(sampleData2)}
            disabled={!isWebViewReady}
          />
          <Button
            title="Update to Data 3"
            onPress={() => updateChartData(sampleData3)}
            disabled={!isWebViewReady}
          />
        </View>
