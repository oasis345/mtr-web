...
export class LightweightChartsService implements ChartService {
  ...
  constructor(adapter: ChartAdapter, chartContainer: HTMLElement | null) {
    if (!chartContainer) {
      const error = new Error(
        '[LightweightChartsService] 오류: 차트를 초기화할 수 없습니다. 유효한 HTML 컨테이너가 필요합니다.',
      );
  ...
  public initialize(initialData: ChartDataPoint[]): void {
    ...
    // 초기 데이터 설정
    const formattedData: LineData[] = initialData.map(d => ({ time: d.time, value: d.value }));
  ...
  public updateData(data: ChartDataPoint[]): void {
    if (!this.series) {
      const error = new Error(
        '[LightweightChartsService] 오류: 데이터를 업데이트할 수 없습니다. 시리즈가 초기화되지 않았습니다.',
      );
      this.adapter.onError(error);
      return;
    }
    const formattedData: LineData[] = data.map(d => ({ time: d.time, value: d.value }));
    this.series.setData(formattedData);
